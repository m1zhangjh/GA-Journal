...
\tabref{tab:GIDE} \newtext{summaries the four versions of GIDE.} \garand~\newtext{is the basic genetic algorithm.} \gads and \gaana~\newtext{are genetic algorithms with one level guided local search.} \gads~\newtext{uses the Domain Score (DS) to guide local search, which has high speed but low accuracy of estimation.} Conversely, \gaana ~\newtext{uses the slower but more accurate Analytic Evaluation (AE) in the local search.} Finally, \gah~\newtext{employs a two-level guided search, combining both DS and AE to balance speed and accuracy.}  
